function map = computeDiskConformalMap(v,f)
% COMPUTEDISKCONFORMALMAP Compute a disk conformal map for an open simply connected surface.
% Author: Joey.Chow
...
% Disk harmonic map
M = computeCotangentLaplacian(v,f);
...
if sum(sum(isnan(map))) ~= 0
    ...
    map = computeTutteMap(v,f,bdy_index,bdy); map_prev = map;
    z = map(:,1) + 1i*map(:,2);
end
...
mu = computeBeltramiCoefficient(map, f, v); 
mu_v = buildFaceToVertexInterpolationMatrix(v,f)*mu;
...
mu = computeBeltramiCoefficient(P, f, v); 

% compute the updated x coordinates
target = P(fixed,1);
A = computeGeneralizedLaplacian(P,f,mu); Ax = A; Ay = A;
b = -Ax(:,fixed)*target;
b(fixed) = target;
...
energy_old = 0;
energy = mean(abs(computeBeltramiCoefficient([real(z_new),imag(z_new),0.*z_new], f, v)));
...
while abs(energy_old-energy) > parameter.threshold
    ...
    mu = computeBeltramiCoefficient([real(z_new),imag(z_new),0.*z_new], f, v);
    ...
    fixed2 = ind(1:max(round(length(z_filled)/parameter.south),...
        min(100,length(z))));
    map_filled = solveLinearBeltrami(...
    ...
    mu_temp = computeBeltramiCoefficient([real(z_final),imag(z_final),0.*z_final],f,v);
    map_temp = solveLinearBeltrami(...
    ...
    energy = mean(abs(computeBeltramiCoefficient(map, f, v)));
    map_opt = map;
