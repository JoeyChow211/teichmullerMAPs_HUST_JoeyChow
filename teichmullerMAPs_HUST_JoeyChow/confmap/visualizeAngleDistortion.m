function visualizeAngleDistortion(v,f,map)
% VISUALIZEANGLEDISTORTION Visualize the angle distortion induced by a mapping.
% Author: Joey.Chow
...
% distortion2 = sum(reshape(abs(distortion),[size(f,1),3]),2);
for i = 1:size(v,1)
    V_distortion(i) = log(mean(abs(distortion(f(:,:)==i))));
end

showTriMesh(v,f,V_distortion');
ylabel(colorbar,'Angle Distortion {ln(|\theta|)}','FontSize',25,'FontName','Times New Roman');
% histogram
figure;
histogram(abs(distortion),0:180);
